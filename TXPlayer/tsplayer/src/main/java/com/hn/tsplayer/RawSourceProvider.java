...
public class RawSourceProvider implements IMediaDataSource {
    private AssetFileDescriptor mDescriptor;

    private byte[] mMediaBytes;

    public RawSourceProvider(AssetFileDescriptor descriptor) {
    ...
    public static RawSourceProvider create(Context context, Uri uri) {
        try {
            AssetFileDescriptor fileDescriptor = context.getContentResolver().openAssetFileDescriptor(uri, "r");
            return new RawSourceProvider(fileDescriptor);
